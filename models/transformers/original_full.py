...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, ref_pts=None, mem_bank=None, mem_bank_pad_mask=None, attn_mask=None):
        ...
        n_dt = len(ref_pts) if attn_mask is None else attn_mask[:,-1].sum().item()

        enc_keys = query_embed[:n_dt].T.view(1,self.d_model,1,-1)
        enc_refs = ref_pts[:n_dt]
        srcs.append(enc_keys)
        masks.append(torch.zeros_like(enc_keys[:,0]).bool())
        pos_embeds.append(torch.zeros_like(enc_keys))
        ...
        # encoder
        memory = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten, enc_refs)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, enc_refs=None):
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        enc_refs = enc_refs[None,:,None,:2] * valid_ratios[-1:,None]
        reference_points = torch.cat(( reference_points[:,:-enc_refs.shape[1]],   enc_refs  ), dim=1)
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        ...
        num_feature_levels=args.num_feature_levels+2,
